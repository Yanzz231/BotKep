...
module.exports = {
  ...
  lewdBomb: async function (n) {
    // Sends a bomb of random images of N value!
    // Contributed by @HanBao#8443 !! Thank you so much !
    if (n) {
      if (n > 20)
        throw new Error(
          `Akaneko - lewdBomb(n) | The parameter you specified is above the maximum of 20!`
        );
      if (isNaN(n))
        throw new Error(
          `Akaneko - lewdBomb(n) | Parameter must be an integer!`
        );
    ...
    const array = await Promise.all(
      params.map((param) => resolve.get("random"))
    );
